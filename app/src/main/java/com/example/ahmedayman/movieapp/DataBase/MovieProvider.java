        ...
        switch (sUriMatcher.match(uri)) {
            case MOVIE: {
                retCursor = mOpenHelper.getReadableDatabase().query(
                        MovieContract.MovieData.TABLE_NAME, projection, selection, selectionArgs,
                        null, null, sortOrder);
                break;
            }
            default:
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (sUriMatcher.match(uri)) {
            case MOVIE: {
                ...
                if (_id > 0) {
                    returnUri = MovieContract.MovieData.buildMovieUri(_id);
                }
                else {
        ...
        switch (sUriMatcher.match(uri)) {
            case MOVIE:
                rowsDeleted = db.delete(
                        MovieContract.MovieData.TABLE_NAME, selection, selectionArgs);
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        }
